...
#+BEGIN_SRC R :tangle R/disambr_stats.r
  ...
  disambr_stats <- function(sets = NULL
      ...
      sets_attr_duration <- 
          sapply(sets_attr, `[[`, "disambr_set_duration")

      dur_mins <- as.numeric(Reduce(`+`,sets_attr_duration))/60
...
#+BEGIN_SRC R :session 
  ...
  ## does not save time when reading files...?
  ## disambr_stats(ds, sets_dir = "../eva-slow-dl01")
#+END_SRC

#+BEGIN_SRC R :results replace table :session none :colnames yes
  files <- dir(".."
             , recursive = TRUE
             , full.names = TRUE
             , pattern = "^disambr_stats\\.rds$")
  stats_list <- lapply(files, readRDS)
  as.matrix(data.table::rbindlist(stats_list))
#+END_SRC
