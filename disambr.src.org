...
#+BEGIN_SRC R :tangle R/disambr_read.r
  disambr_rbind_wos <- function(tables_list, tables_recipes) {
      ...
      attributes(publication_table)$disambr_entity <- "publication"
      attributes(publication_table)$disambr_entity_id_reference <- "self"
      attributes(publication_table)$disambr_set_type <- "different_entities"
      attributes(publication_table)$disambr_set_coefficient <- 1
      attributes(publication_table)$disambr_set_name <- "wos_records_tsv_export"
      attributes(publication_table)$disambr_set_collection <- "single_set_table"
      ## add files recipies
...
#+BEGIN_SRC R :tangle R/disambr_eva.r
  ## This file defines procedures for EVA algorithm
  ...
  disambr_set_co_authors <- function(sets, procedures = NULL) {
      ...
      if(!is.list(sets)) stop("disambr: sets should be list!")
  ...
  disambr_set_not_co_authors <- function(sets, procedures = NULL) {
      message("disambr: Starting disambr_set_not_co_authors...")
      if(!is.list(sets)) stop("disambr: sets should be list!")
      data.set <- disambr_subsets(sets, list( "person"))
      return.set <- data.set %>%
          {split(1:nrow(.), .$paper_id)}
