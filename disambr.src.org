...
*** disabmr_get_first_data_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
  disabmr_get_first_data_set <- function() {}
#+END_SRC

*** disabmr_get_last_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_get_last_set <- function() {}
#+END_SRC

*** disabmr_get_first_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_get_first_set <- function() {}
#+END_SRC

*** disabmr_get_merged_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_get_merged_set <- function() {}
#+END_SRC

*** disabmr_get_last_sans_merged_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_get_last_sans_merged_set <- function() {}
#+END_SRC

*** disabmr_get_last_unmerged_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_get_last_unmerged_set <- function() {}
#+END_SRC
...
#+BEGIN_SRC R :tangle R/disambr_sets.r
  ...
  ##' @param check_attr_names Whether to check for short names of attributes. See `attr_names_prefix`.
  ...
  disambr_in_sets <- function(sets
      ...
      if(isTRUE(check_attr_names)) {
          ...
          filter_sets <- function(attr_name, attr_value) {
              sapply(lapply(sets, attr, attr_name), `[`, 1) %in% attr_value
          }
      }
...
**** disabmr_in_first_data_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
  disabmr_in_first_data_set <- function(sets, ...) {
      set_num <- disambr_in_sets(sets, recipie = "disambr_eject_authors", ...)
...
**** disabmr_in_last_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_in_last_set <- function() {}
#+END_SRC

**** disabmr_in_first_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_in_first_set <- function() {}
#+END_SRC

**** disabmr_in_merged_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_in_merged_set <- function() {}
#+END_SRC

**** disabmr_in_last_sans_merged_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_in_last_sans_merged_set <- function() {}
#+END_SRC

**** disabmr_in_last_unmerged_person_set
#+BEGIN_SRC R :tangle R/disambr_sets.r
disabmr_in_last_unmerged_person_set <- function() {}
#+END_SRC
...
#+BEGIN_SRC R :tangle R/disambr_eject_wos.r
  ...
  ##' 
  ##' @md 
  disambr_eject_authors_parse_au <- function(author_name) {
      author_last_name <-
...
#+BEGIN_SRC R :tangle R/disambr_eject_wos.r
  ...
  ##' @return Data.table
  disambr_eject_authors_parse_af <- function(name) {
...
#+BEGIN_SRC R :tangle R/disambr_eject_wos.r
  ...
  disambr_eject_authors_parse_em <- function(emails
                                           , record_au_table
                                           , record_rp_table) {
...
#+BEGIN_SRC R :tangle R/disambr_eject_wos.r
  ...
  ## "[Wang, Menglei; Li, Shunyi; Zhu, Rencheng; Zhang, Ruiqin] Zhengzhou Univ, Sch Ecol & Environm, Zhengzhou 450001, Peoples R China; [Wang, Menglei] Zhengzhou Univ, Sch Chem Engn, Zhengzhou 450001, Peoples R China; [Zu, Lei; Wang, Yunjing; Bao, Xiaofeng] Chinese Res Inst Environm Sci, State Environm Protect Key Lab Vehicle Emiss Cont, Beijing 100012, Peoples R China" %>%
  ## disambr_eject_authors_parse_c1(disambr_eject_authors_parse_af("Wang, Menglei; Zu, Lei; Wang, Yunjing; Bao, Xiaofeng"))
#+END_SRC
*** disambr_eject_authors_parse_oi
:PROPERTIES:
:ID:       org:jygfnb60jti0
:END:
#+BEGIN_SRC R :tangle R/disambr_eject_wos.r
  ...
  ##' 
  ##' @md 
  disambr_eject_authors_parse_oi <- function(authors
                                            , table_af = NULL) {
      author_full_name <-
...
##' Parses RI column of WoS saved records export
  ...
  ##' 
  ##' @md 
  disambr_eject_authors_parse_ri <- function(authors
                                            , table_af = NULL) {
      author_full_name <-
          stringi::stri_extract_first_regex(authors, "^[^/]+")
  ...
  disambr_eject_authors <- function(wos_data_table
      ...
      if(any(c("author_email") %in% list_of_author_fields) &&
         all(c("AU", "EM", "RP") %in% names(wos_data_table))) {
          ...
          pbmapply(disambr_eject_authors_parse_em
      ...
      if(any(c("author_affiliations") %in% list_of_author_fields) &&
         all(c("C1", "AF") %in% names(wos_data_table))) {
          ...
          authors_tables$c1 <-
              pbmapply(disambr_eject_authors_parse_c1
                   , wos_data_table$C1
                   , authors_tables$af
                   , SIMPLIFY = FALSE)
      }
      ...
      if(any(c("author_researcher_id") %in% list_of_author_fields) &&
         all(c("RI", "AF") %in% names(wos_data_table))) {
          ...
          authors_tables$ri <-
              pbmapply(disambr_eject_authors_parse_ri
      ...
      if(any(c("author_orcid") %in% list_of_author_fields) &&
         all(c("OI", "AF") %in% names(wos_data_table))) {
          ...
          authors_tables$oi <-
              pbmapply(disambr_eject_authors_parse_oi
...
#+END_SRC
*** disambr_eject_citations_name                               
:PROPERTIES:
:ID:       org:to0gnb60jti0
:END:
#+BEGIN_SRC R :tangle R/disambr_eject_wos.r
  disambr_eject_citations_name <- function(pub_table, ref_table) {
      ...
      disambr_setattr(
        ...
        , disambr_recipe = "disambr_eject_citations_name")
      return(cit_table)
  }
  ...
  ## disambr_eject_citations_name(dt[[1]], dt[[3]])

  ## disambr_eject_citations_name(dt[[1]], dt[[3]])$first_author_last_name %>% sapply(is.na) %>% sum
...
*** disambr_process_files_data
:PROPERTIES:
:ID:       org:33ignb60jti0
:END:
#+BEGIN_SRC R :tangle R/disambr_read.r
  disambr_process_files_data <- function(files_data_list
                                       , verbose = getOption("disambr_verbose")
                                       , drop_ejected = FALSE) {
        ...
        if (length(processabe_data) != 0) {
            ...
            if (length(unique(processabe_data_headers)) == 1) {
                ...
                if(verbose) message("- rbinding wos tables..")
                wos_publication <-
                    disambr_rbind_wos(files_data_list, processabe_data_recipes)
                if(verbose) message("- ejecting wos authors..")
                wos_author <- disambr_eject_authors(wos_publication
                , verbose = verbose)
                if(drop_ejected) {
                ...
                if(verbose) message("- ejecting wos references..")
                ...
                if(verbose) message("- ejecting author-year citations table..")
                citation_name_table <-
                    disambr_eject_citations_name(wos_publication, wos_reference)
...
#+BEGIN_SRC R
  disambr_set_template <- function(sets
      ...
      disambr_mess_start()
      ## get sets
      input_set <- disabmr_get_last_unmerged(sets)
      input_set <- disabmr_get_last_minus_merged(sets)
      merged_set <- disabmr_get_all_merged(sets)
      ## return_set <- disabmr_get_last_saved_set(...)
      return_set <- disabmr_get_return_set(sets, ...)
      ## procedure
